...
handle_request([], <<"POST">>, Req) ->
    case handler_helper:verify_token(Req) of
        ...
        {ok, UserId} ->
            ...
            {<<"name">>, GroupName} = lists:keyfind(<<"name">>, 1, PostVals),
            {ok, Members} = get_members(PostVals),
...
handle_request([<<"member">>], <<"DELETE">>, Req) ->
    case handler_helper:verify_token(Req) of
        {error, TomlBin} ->
            ok;
        {ok, UserId} ->
            {ok, PostVals, _} = cowboy_req:body_qs(Req),
            case utility:check_parameters([<<"g_id">>], PostVals) of
                {ok, [GroupIdBin]} ->
...
handle_request([GroupIdBin], <<"DELETE">>, Req) ->
    ...
    cowboy_req:reply(200, [], TomlBin, Req);
handle_request([<<"member">>], <<"POST">>, Req) ->
    case handler_helper:verify_token(Req) of
        {error, TomlBin} ->
            ok;
        {ok, UserId} ->
            {ok, PostVals, _} = cowboy_req:body_qs(Req),
            case utility:check_parameters([<<"g_id">>, <<"g_key">>], PostVals) of
                {ok, [GroupIdBin, Key]} ->
                    GroupId = erlang:binary_to_integer(GroupIdBin),
    ...
    cowboy_req:reply(200, [], TomlBin, Req);
handle_request([<<"member">>, MemberIdBin], <<"POST">>, Req) ->
    case handler_helper:verify_token(Req) of
        {error, TomlBin} ->
            ok;
        {ok, UserId} ->
